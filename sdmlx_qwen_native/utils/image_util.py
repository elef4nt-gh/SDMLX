...
from sdmlx_qwen_native.utils.box_values import AbsoluteBoxValues, BoxValues
from sdmlx_qwen_native.utils.generated_image import GeneratedImage
from sdmlx_qwen_native.utils.metadata_builder import MetadataBuilder

if TYPE_CHECKING:
    from sdmlx_qwen_native.models.flux.variants.concept_attention.attention_data import ConceptHeatmap
...
class ImageUtil:
    ...
    @staticmethod
    def _decoded_to_pil(images: mx.array) -> PIL.Image.Image:
        if len(images.shape) == 5:
            images = mx.squeeze(images, axis=2)
        images = mx.transpose(images, (0, 2, 3, 1))
        images = mx.clip(((images / 2.0) + 0.5) * 255.0 + 0.5, 0, 255)
        images = mx.array.astype(images, mx.uint8)
        mx.eval(images)
        mx.clear_cache()
        pil_images = [PIL.Image.fromarray(image) for image in np.array(images)]
        return pil_images[0]
