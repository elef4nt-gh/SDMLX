...
class MetadataBuilder:
    ...
    def build_xmp_packet(metadata: dict) -> str:
        ...
        version = metadata.get("sdmlx_qwen_native_version", "unknown")
...
<rdf:Description rdf:about=""
    ...
    xmlns:sdmlx_qwen_native="http://ns.sdmlx_qwen_native.ai/1.0/">
    <dc:description><rdf:Alt><rdf:li xml:lang="x-default">{prompt_escaped}</rdf:li></rdf:Alt></dc:description>
    <dc:creator><rdf:Seq><rdf:li>SDMLX Qwen</rdf:li></rdf:Seq></dc:creator>
    <dc:rights><rdf:Alt><rdf:li xml:lang="x-default">AI Generated Content</rdf:li></rdf:Alt></dc:rights>
    <xmp:CreatorTool>SDMLX Qwen {version}</xmp:CreatorTool>
    <photoshop:Category>ART</photoshop:Category>
    <photoshop:Credit>Generated by SDMLX Qwen</photoshop:Credit>"""

        # Add technical parameters to XMP
        if "seed" in metadata:
            xmp_packet += f"\n    <sdmlx_qwen_native:seed>{metadata['seed']}</sdmlx_qwen_native:seed>"
        if "steps" in metadata:
            xmp_packet += f"\n    <sdmlx_qwen_native:steps>{metadata['steps']}</sdmlx_qwen_native:steps>"
        if "guidance" in metadata:
            xmp_packet += f"\n    <sdmlx_qwen_native:guidance>{metadata['guidance']}</sdmlx_qwen_native:guidance>"
        if model_name:
            xmp_packet += f"\n    <sdmlx_qwen_native:model>{model_name}</sdmlx_qwen_native:model>"
        if lora_info:
            xmp_packet += f"\n    <sdmlx_qwen_native:loras>{lora_info}</sdmlx_qwen_native:loras>"
        # Keep backward-compat for older key name, prefer current 'generation_time_seconds'
        if "generation_time_seconds" in metadata:
            xmp_packet += f"\n    <sdmlx_qwen_native:generationTimeSeconds>{metadata['generation_time_seconds']}</sdmlx_qwen_native:generationTimeSeconds>"
        elif "generation_time" in metadata:
            xmp_packet += f"\n    <sdmlx_qwen_native:generationTime>{metadata['generation_time']}</sdmlx_qwen_native:generationTime>"
...
<?xpacket end="w"?>"""
    ...
    def build_iptc_binary(metadata: dict) -> bytes:
        ...
        if "prompt" in metadata:
            ...
            if (
                ...
                summary = (
                    ...
                    "Full prompt preserved in SDMLX Qwen metadata (EXIF UserComment) and may be saved as a sidecar .json."
                )
        ...
        keywords = ["AI", "Generated", "SDMLX Qwen"]
        if "seed" in metadata:
