...
from sdmlx_qwen_native.models.common.resolution.config_resolution import ConfigResolution
...
class ModelConfig:
    ...
    def copy_with_model_identity(
        ...
        return ModelConfig(
            ...
            aliases=aliases if aliases is not None else list(self.aliases),
            model_name=model_name,
            base_model=base_model,
            controlnet_model=self.controlnet_model,
            custom_transformer_model=self.custom_transformer_model,
            num_train_steps=self.num_train_steps,
            max_sequence_length=self.max_sequence_length,
            supports_guidance=self.supports_guidance,
            requires_sigma_shift=self.requires_sigma_shift,
            transformer_overrides=dict(self.transformer_overrides),
            text_encoder_overrides=dict(self.text_encoder_overrides),
            sigma_base_shift=self.sigma_base_shift,
            sigma_max_shift=self.sigma_max_shift,
            sigma_base_seq_len=self.sigma_base_seq_len,
            sigma_max_seq_len=self.sigma_max_seq_len,
            sigma_shift_terminal=self.sigma_shift_terminal,
            lora_training_steps=self.lora_training_steps,
            lora_training_guidance=self.lora_training_guidance,
