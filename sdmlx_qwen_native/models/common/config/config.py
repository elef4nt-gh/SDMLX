...
from sdmlx_qwen_native.models.common.schedulers import SCHEDULER_REGISTRY, try_import_external_scheduler
from sdmlx_qwen_native.models.common.schedulers.linear_scheduler import LinearScheduler
from sdmlx_qwen_native.utils.dimension_resolver import DimensionResolver
from sdmlx_qwen_native.utils.scale_factor import ScaleFactor
...
class Config:
    def __init__(
        ...
        # Ensure dimensions are multiples of 16. FLUX/DiT latent grids require this,
        # but callers may intentionally pass image-derived dimensions.
        if width % 16 != 0 or height % 16 != 0:
            logger.debug("Width and height should be multiples of 16. Rounding down.")
    ...
    @property
    def flow_shift(self) -> float | None:
    ...
    def scheduler(self):
        ...
        if self._scheduler_str == "linear":
            ...
            raise NotImplementedError(f"The scheduler {self._scheduler_str!r} is not implemented by sdmlx_qwen_native.")

        if self._flow_shift is not None and hasattr(self._scheduler, "set_mu"):
            self._scheduler.set_mu(float(self._flow_shift))
        elif hasattr(self._scheduler, "set_image_seq_len") and self.model_config.requires_sigma_shift:
