        ...
        for hf_key, hf_tensor in hf_weights.items():
            ...
            if targets:
                for mlx_path, transform in targets:
                    ...
                    if hf_key.endswith(".weight") and mlx_path.endswith(".weight"):
                        hf_base = hf_key[: -len(".weight")]
                        mlx_base = mlx_path[: -len(".weight")]
                        for aux_name in ("scales", "biases"):
                            aux_key = f"{hf_base}.{aux_name}"
                            if aux_key in hf_weights:
                                WeightMapper._set_nested_value(mapped_weights, f"{mlx_base}.{aux_name}", hf_weights[aux_key])
                                mapped_count += 1
            else:
