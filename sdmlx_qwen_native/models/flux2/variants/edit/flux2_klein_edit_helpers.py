...
from sdmlx_qwen_native.models.flux2.model.flux2_text_encoder.prompt_encoder import Flux2PromptEncoder
    ...
    def prepare_reference_image_conditioning(
        *,
        ...
        for i, p in enumerate(image_paths):
            ref_width, ref_height = reference_sizes[i] if reference_sizes is not None else (width, height)
            mark(f"generate.reference_{i + 1}.encode_start")
            ...
            mark(f"generate.reference_{i + 1}.vae_encode_returned")
            ...
            mark(f"generate.reference_{i + 1}.packed")

            # FLUX.2 uses Comfy's `index` reference method by default:
            # each reference image gets a distinct time/index plane, scaled by
            # 10. Draw Things' FLUX.2 rotary embedding uses the same 10,20,...
            # sequence for reference images. The generic FLUX `offset` layout
            # collapses multi-image semantics for FLUX.2 Klein.
            packed_latents_list.append(Flux2LatentCreator.pack_latents(encoded))
            ids_list.append(
                Flux2LatentCreator.prepare_grid_ids(
                    encoded,
                    t_coord=10 + 10 * i,
                )
            )
        ...
        mark("generate.references.concatenated")
        ...
        mark("generate.references.ready")
