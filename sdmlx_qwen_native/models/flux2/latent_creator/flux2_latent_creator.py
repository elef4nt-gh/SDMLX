...
class Flux2LatentCreator:
    @staticmethod
    def _comfy_noise(shape: tuple[int, ...], seed: int) -> mx.array:
        try:
            import torch
        except Exception:
            return mx.random.normal(shape=shape, key=mx.random.key(seed)).astype(mx.float32)

        generator = torch.manual_seed(int(seed))
        noise = torch.randn(shape, dtype=torch.float32, generator=generator, device="cpu")
        return mx.array(noise.numpy(), dtype=mx.float32)
    ...
    def prepare_grid_ids(
        ...
        *,
        ...
        h_ids = mx.arange(height, dtype=mx.int32) + int(h_offset)
        w_ids = mx.arange(width, dtype=mx.int32) + int(w_offset)
    ...
    def prepare_latents(
        ...
        latents = Flux2LatentCreator._comfy_noise(
