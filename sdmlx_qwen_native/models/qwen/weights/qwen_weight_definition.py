...
from sdmlx_qwen_native.models.qwen.weights.qwen_weight_mapping import QwenWeightMapping
...
class QwenWeightDefinition:
    ...
    @staticmethod
    def _strip_transformer_prefix(key: str) -> str | None:
        aio_prefix = "model.diffusion_model."
        if key.startswith(aio_prefix):
            return key[len(aio_prefix) :]
        if any(key.startswith(prefix) for prefix in QwenWeightDefinition._TRANSFORMER_PREFIXES if prefix != aio_prefix):
            return key
        return None

    @staticmethod
    def _strip_text_encoder_prefix(key: str) -> str | None:
        aio_prefix = "text_encoders.qwen25_7b.transformer."
        if key.startswith(aio_prefix):
            return key[len(aio_prefix) :]
        if key.startswith(("model.embed_tokens.", "model.layers.", "model.norm.", "visual.")):
            return key
        return None

    @staticmethod
    def _strip_vae_prefix(key: str) -> str | None:
        aio_prefix = "vae."
        if key.startswith(aio_prefix):
            return key[len(aio_prefix) :]
        if any(key.startswith(prefix) for prefix in QwenWeightDefinition._VAE_PREFIXES if prefix != aio_prefix):
            return key
        return None
    ...
    def get_components() -> List[ComponentDefinition]:
        return [
            ComponentDefinition(
                name="vae",
                hf_subdir="vae",
                loading_mode="single_mlx",
                weight_prefix_filters=QwenWeightDefinition._VAE_PREFIXES,
                key_transform=QwenWeightDefinition._strip_vae_prefix,
            ...
            ComponentDefinition(
                ...
                weight_prefix_filters=QwenWeightDefinition._TRANSFORMER_PREFIXES,
                key_transform=QwenWeightDefinition._strip_transformer_prefix,
            ...
            ComponentDefinition(
                ...
                weight_prefix_filters=QwenWeightDefinition._TEXT_ENCODER_PREFIXES,
                key_transform=QwenWeightDefinition._strip_text_encoder_prefix,
    ...
    def _transformer_quantization_predicate_for_loaded_weights(weights: LoadedWeights, bits: int | None):
        ...
        if bits == 8:
            return QwenWeightDefinition._loaded_transformer_quantization_predicate(transformer)
    ...
    @staticmethod
    def _loaded_transformer_quantization_predicate(transformer: dict):
        def predicate(path: str, module, bits: int | None = None):
            if not hasattr(module, "to_quantized"):
                return False

            path_bits = QwenWeightDefinition._quantized_linear_bits(transformer, path)
            if path_bits is None:
                return False
