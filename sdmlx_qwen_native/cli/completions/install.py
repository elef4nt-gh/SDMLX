...
from sdmlx_qwen_native.cli.completions.generator import CompletionGenerator
...
def check_installation():
    print("Checking sdmlx_qwen_native completions installation...\n")
    ...
    if not found_locations:
        print("❌ No sdmlx_qwen_native completion file found in $fpath")
        print("\nSearched in:")
        for path_str in fpath[:5]:  # Show first 5 locations
            print(f"  - {path_str}")
        print("\nRun 'sdmlx_qwen_native-completions' to install completions")
        return False
    ...
    print("  2. Type: sdmlx_qwen_native-generate --<TAB>")
...
def main():
    parser = argparse.ArgumentParser(
        description="Install or generate ZSH completions for sdmlx_qwen_native commands",
...
Examples:
  ...
  # Generate to stdout
  sdmlx_qwen_native-completions --print

  # Install to specific directory
  sdmlx_qwen_native-completions --dir ~/.config/zsh/completions

  # Update existing completion
  sdmlx_qwen_native-completions --update

  # Check if completions are properly installed
  sdmlx_qwen_native-completions --check
""",
    )
